...
function Card({ post, featured = false, lp }) {
  ...
  return (
    <motion.div
      ...
      <Link to={lp(`/blog/${post.slug}`)} style={{
      ...
      onMouseLeave={(e) => e.currentTarget.style.opacity = '1'}>
        <div style={{
          width: '100%', aspectRatio: featured ? '16/9' : '4/3',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <h2 style={{ fontSize: featured ? 28 : 20, fontWeight: 500, margin: '10px 0 0', letterSpacing: '-0.5px', lineHeight: 1.2, color: 'var(--okc-text-primary)' }}>
            {post.title}
          </h2>
          {featured && (
...
export default function BlogIndex() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--okc-bg-white)' }}>
      {/* Nav */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--okc-border)' }}>
      ...
      <header style={{ padding: '80px 56px 64px', maxWidth: 1440, margin: '0 auto', borderBottom: '1px solid var(--okc-border)' }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1, ease }}
        ...
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2, ease }}
          style={{ fontSize: 19, lineHeight: 1.6, color: 'var(--okc-text-secondary)', marginTop: 28, maxWidth: '52ch' }}>
          {labels.lead}
        </motion.p>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3, ease }}
      ...
      <main style={{ maxWidth: 1440, margin: '0 auto', padding: '64px 56px 120px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 32 }}>
          <Card post={featured} featured lp={lp} />
          {rest.map((p) => <Card key={p.slug} post={p} lp={lp} />)}
        </div>
      </main>
      ...
      <footer style={{ borderTop: '1px solid var(--okc-border)', padding: '40px 56px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: 1440, margin: '0 auto' }}>
        ...
        <Link to={lp('/')} style={{ fontSize: 13, fontFamily: 'var(--okc-font-mono)', textTransform: 'uppercase', letterSpacing: 1, color: 'var(--okc-text-primary)', borderBottom: '1px solid var(--okc-text-primary)', paddingBottom: 2 }}>
          {labels.home}
        </Link>
      </footer>
    </div>
